...
const CoreValueCard: React.FC<CoreValueCardProps> = ({ imageurl, title, description }) => {
  return (
    <div
      className="w-[95%] xl:w-[80%] lg:w-[82%] md:w-[85%] sm:w-[88%] h-[90%] flex items-center justify-center bg-gray-50 rounded-2xl shadow-lg transition-transform duration-300 hover:scale-105  flex-col xl:flex-row lg:flex-row md:flex-row sm:flex-row "
    >
      <div className='w-[20%] h-[100px]  xl:h-full lg:h-full md:h-full sm:h-[90%] flex items-center justify-center '>
        <Image
          src={imageurl}
          alt={title}
          width={96}
          height={96}
          className="xl:w-[50%] xl:h-[50%] lg:w-[50%] lg:h-[50%] md:w-[50%] md:h-[50%] sm:w-[70%] sm:h-[70%]  rounded-lg color-red"
        />
      </div>
